...
/// Dispatch a webhook event asynchronously. This function spawns a background task
/// so it never blocks the API response path.
pub async fn dispatch_event(
    ...
    tokio::spawn(async move {
        ...
        let endpoints = match endpoints {
            Ok(e) => e,
            Err(err) => {
                tracing::error!(error = %err, "Failed to fetch webhook endpoints");
                return;
            }
        };
        ...
        for endpoint in endpoints {
            ...
            let signature = sign_payload(&payload_str, &endpoint.secret);
            ...
            .await
            {
                tracing::error!(error = %e, endpoint_id = %endpoint.id, "Failed to persist webhook event");
                continue;
            }

            deliver_webhook(event_id, &endpoint.url, &payload_str, &signature, &pool).await;
        }
    });
}
