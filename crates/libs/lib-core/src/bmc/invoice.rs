...
impl InvoiceBmc {
    pub async fn create(
        ...
        let result = Self::create_in_tx(&mut tx, ctx, data).await?;
        tx.commit().await?;
        Ok(result)
    }

    /// Create an invoice within an existing transaction.
    /// Caller is responsible for committing. Used by handlers that need to
    /// enqueue webhook events in the same transaction (outbox pattern).
    ...
    /// Returns the updated Invoice so callers (e.g. webhooks) always have the
    /// committed state — no stale data possible.
    pub async fn mark_paid(pool: &PgPool, id: Uuid) -> Result<Invoice> {
        sqlx::query_as::<_, Invoice>(
    ...
    /// Transition invoice to paid within an existing transaction.
    /// Used by the payment handler for atomic state change + webhook enqueue.
